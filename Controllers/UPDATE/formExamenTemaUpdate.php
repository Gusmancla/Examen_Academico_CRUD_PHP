...
$id =$_GET['id'];
$sentencia = $db->prepare("SELECT * FROM ExamenTema WHERE IDEvaluacion=?");#cambiar el where por la columna correspondiente.
...
<body>
    <center>
        <form action="UpdateExamenTema.php" method="post">
            <h2>Ingrese nuevo registro.</h2>
            <table>                
                <tr>
                    <td>ID Evaluaciones: <input type="number" name="idEval" require value="<?php echo $OBJ->IDEvaluacion; ?>"></td>
                </tr>
                <tr>
                    <td>ID Tema: <input type="number" name="idTem" require value="<?php echo $OBJ->IDTema; ?>"></td>
                </tr>
                <tr>
