...
if ($db) {
    ...
    $codigoPractica=$_POST['codigoDePractica'];
    $titulo=$_POST['Titulo'];
    $NivelDificultad=$_POST['nivelDeDificultad'];
    $Fecha=$_POST['fechaDeInicio'];

    #Sentencia Sql para insertar registros en la base de datos.
    $sentencia = $db->prepare("INSERT INTO Practicas(CodigoPractica,Titulo,NivelDificultad,Fecha) VALUES (?,?,?,?)");
    # ejecuta la sentencia  por medio del resultado por las variables nuevas creadas.
    $resultado = $sentencia->execute([$codigoPractica,$titulo,$NivelDificultad,$Fecha]);
